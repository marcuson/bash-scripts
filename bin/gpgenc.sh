...
readonly script_description="Encrypt/decrypt file(s) using GPG"
...
option|p|PASSPHRASE|Passphrase to use |
option|L|LOG_DIR|folder for log files |$HOME/log/$script_prefix
option|T|TMP_DIR|folder for temp files|/tmp/$script_prefix
choice|1|action|action to perform|encrypt,decrypt,env
param|1|IN_FILE|Input file
" -v -e '^#' -e '^\s*$'
}
...
function Script:main() {
    ...
    encrypt)
        #TIP: use «$script_prefix encrypt» to encrypt a file using gpg
        #TIP:> $script_prefix encrypt -p <passphrase> file
        do_encrypt
        ;;

    decrypt)
        #TIP: use «$script_prefix decrypt» to decrypt a file using gpg
        #TIP:> $script_prefix decrypt -p <passphrase> file
        do_decrypt
        ;;
...
function do_encrypt() {
    ...
    IO:success "File $in_f encrypted"
}
...
function do_decrypt() {
    ...
    IO:success "File $in_f decrypted"
}
